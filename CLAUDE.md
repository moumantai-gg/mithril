...
- **Instrumentation is not optional — it's the contract.** Every new type that owns lifecycle, state, IO, or a decision branch MUST be instrumentable from day one. Concretely: (a) take `ILogger` / `ILogger<T>` (or `ILoggerFactory` and create a named-category logger inside) in the constructor — `ILogger?` is allowed ONLY for shared types whose test fixtures don't wire DI, and even then the production registration must always wire one; (b) emit `LogTrace` at every branch / decision point a future investigator might need to see (probe kick-off, capture skip, gate reject, fallback fire), `LogInformation` at lifecycle milestones (state transition, async completion with result), `LogWarning` on the failure / safe-degrade paths with the actual reason as a named property, `LogError` only when an exception is the cause; (c) when the type does duration / distribution / rate measurement, add a `Counter` / `Histogram` to the canonical `MithrilMeters` static AND an `ActivitySource` span via `MithrilActivitySources` — span and meter together answer "how often / how long / what shape" that logs alone don't. **If you can't diagnose a failure from the logs without reading code, the type is missing instrumentation — that's a defect, not a future improvement.** mithril#1107's `LiveMapViewService` shipped with NO logger because nothing made it required; every probe failed silently and took multiple verify sessions to even find the symptom. Don't repeat that. See the **Logging** + **Telemetry** + **Instrumentation** bullets above for the API; this bullet exists because those alone weren't preventing the gap.
