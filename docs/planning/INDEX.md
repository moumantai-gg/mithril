...
| [sidecar-rgba-alpha-surface](sidecar-rgba-alpha-surface/) | active | _to file_ | Extend the #931 asset-extractor sidecar contract + `IBaseTextureProvider` to surface texture alpha (separate per-area cache file) so `map-calibration-deviation-mask-1116` can compute a floor-boundary mask |
