...
| D11 | **Tag descriptors land in a NEW Legolas-owned provider**, not in `MithrilSharedTagDescriptors`. Create [`src/Legolas.Module/Diagnostics/LegolasCalibrationTagDescriptors.cs`](../../../src/Legolas.Module/Diagnostics/) implementing `ITagDescriptorProvider`; register via DI in `LegolasModule.Register`. | `MithrilSharedTagDescriptors` lives in `Mithril.Shared`, which cannot reference `Legolas.Module` — layering violation. Descriptor entries are scope-keyed by source/meter name (3rd arg) per [`MithrilSharedTagDescriptors.cs`](../../../src/Mithril.Shared/Diagnostics/Telemetry/MithrilSharedTagDescriptors.cs); the existing `outcome` entry there is scoped to `Mithril.Reference`. Our `outcome` on `Mithril.Legolas.Calibration` is a separate descriptor row, not a reuse. Every (key, scope) pair is its own row. |
...
New tag keys introduced by this pass: `area`, `scene.asset_key`, `scene.parent_area_key`, `cal.source`, `cal.frame`, `cal.residual_px`, `cal.refs`, `cal.path` (already exists), `consumer`, `outcome`, `frame`, `refs_count`, `ghosts_built`, `from`, `to`.
