...
| **D5** | Dead-code removal: in this PR or follow-up | **A** — Atomic PR deletes #988 monotonicity gate, #1005 regime guard, and `AreaCalibration.LocatorScale` field. No `SchemaVersion` bump (old JSON loads with the unknown property silently ignored). Tests for both gates go with them. | Matches #1041's "single atomic PR" precedent. The gate + regime guard + field exist *only* to support multi-attempt re-solve; compose-don't-resolve makes that path nonexistent. Leaving them as dead or `[Obsolete]` invites misuse by the next person to touch this surface. |
