...
**Goal:** Replace the locate stage (NCC scale ladder + ECC sub-pixel refine) with an ORB + BFMatcher + Lowe-ratio + `Cv2.EstimateAffinePartial2D` (RANSAC) pipeline. The cutover wins on **two co-equal axes** (spec §"Goal"): (1) **correctness** — robust to fog of war, map-pin occlusion, and non-coastline maps; the Kur live capture goes from "wrong rect, rejected at NCC 0.473" to "sub-pixel correct, 97.9% inlier ratio"; (2) **time-to-feedback** — the same Kur attempt's session log shows the *failure path* takes **3.68 s** today; the prototype runs in ~200–300 ms (~15× speedup), and descriptor caching cuts steady-state to ~80–150 ms (~25× steady-state speedup). Both wins fall out of the same change. Hard cutover — no NCC fallback.
